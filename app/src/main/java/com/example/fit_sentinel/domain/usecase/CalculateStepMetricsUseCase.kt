...
class CalculateStepMetricsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(steps: Int, durationMillis: Long? = null): StepMetrics {
        ...
        val estimatedTimeMinutes = if (durationMillis != null) {
            TimeUnit.MILLISECONDS.toMinutes(durationMillis).toInt()
        } else {
            (validDistanceKm / FALLBACK_WALKING_SPEED_KMH * 60).toInt()
        }

        return StepMetrics(
            distanceKm = validDistanceKm,
            caloriesBurned = caloriesBurned.coerceAtLeast(0.0),
            estimatedTimeMinutes = estimatedTimeMinutes.coerceAtLeast(0)
        )
    }
